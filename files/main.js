...
class ShoppingCart {
    ...
    add(book) {
        ...
        new ElementCreator("li").id(book.cartId)
            .append(new ElementCreator("a").text(book.title).listener("click", ()=> { 
                this.delete(book)
                this.showSum();
            }))
...
class BookStore {
    ...
    addCategoryToDOM(category) {
        new ElementCreator("section")
            .id(category.name)
            //.append(new ElementCreator("h1").text(category.title).id("categoryHeading"))
        ...
        for (const category of data){
            ...
            a.addEventListener("click", ()=> {
                fetch("http://localhost:3000/api/categories/" + category.name + "/books")
                        ...
                        .then(res => res.json())
                        .then(json => bookStore.addToDOM(category, json));
            })
        }
        document.getElementById("html5").click();
    });


});

function enlargeShoppingCart() {

    if (document.getElementById('aside').style.height!='40%') {
        document.getElementById('aside').style.height = '40%';
        document.getElementById('enlarge').innerHTML = "Close";
        document.getElementById('aside').style.backgroundColor = "rgb(161, 220, 131)";
        document.getElementById('footer').style.backgroundColor = "rgb(161, 220, 131)";
        document.getElementById('aside').style.borderTopRightRadius = "10px";
    } else {
        document.getElementById('aside').style.height = '110px';
        document.getElementById('enlarge').innerHTML = "View";
        document.getElementById('aside').style.backgroundColor = "rgb(241, 218, 111)";
        document.getElementById('footer').style.backgroundColor = "rgb(241, 218, 111)";
        document.getElementById('aside').style.borderTopRightRadius = "0px";
    }
